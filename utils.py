...
def restart_from_checkpoint(ckp_path, run_variables=None, **kwargs):
    ...
    try:
        ...
        checkpoint = torch.load(
            ckp_path, map_location='cpu', weights_only=False)
    ...
    if run_variables is not None:
        for var_name in run_variables:
            if var_name in checkpoint and var_name in globals():
                globals()[var_name] = checkpoint[var_name]
    ...
    for key, value in kwargs.items():
        if key in checkpoint and value is not None:
            try:
                ...
                print(f"=> failed to load '{key}' from checkpoint: {e}")
    ...
    def forward(self, x):
        ...
        for end_idx in idx_crops:
            ...
            for i in range(start_idx, end_idx):
                _x = self.backbone(x[i])
                # Reshape if needed - ensure _x is 2D
                if len(_x.shape) > 2:
            ...
            mean, var = self.head(_out)
            output.append((mean, var))
            start_idx = end_idx

        # Return mean and variance for global views only
